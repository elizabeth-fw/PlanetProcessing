...
class PlanetScope8Band(CloudClearBase):
    def __init__(self, tmp_dir, output_dir, aoi):
        ...
        self.dark_threshold = 0.03  # Default threshold (5% reflectance)
    ...
    def udm_buffer_mask(self, udm_file, analytic_file, buffer_size=3):
        ...
        for band_idx in range(1, 7):  # Check bands 2-7 (snow, shadow, haze, cloud, etc.)
            unusable_mask = np.logical_or(unusable_mask,
                                          udm[band_idx] == 1)  # Mark pixels as unusable if any band indicates it
    ...
    def apply_cs_buffer_mask(self, analytic_file, buffer_type="low"):
        ...
        # Combine masks
        final_mask = ~buffered_cloud_mask
